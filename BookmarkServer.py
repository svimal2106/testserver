...
def CheckURI(uri, timeout=5):
    ...
    try:
        ...
        print("Connection error on URL " + uri)
        return False

    except:
        print("Unknown error on trying to connect to " + uri)
        return False
...
class Shortener(http.server.BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if "longuri" not in params or "shortname" not in params:
            ...
            self.send_header('Content-type', 'text/plain; charset=utf-8')
            self.end_headers()
            self.wfile.write("Please specify longuri and shorturi in request".encode())
        ...
        if CheckURI(longuri):
            ...
            self.send_header('Content-type', 'text/plain; charset=utf-8')
            self.end_headers()
            self.wfile.write("Please specify a valid longuri in request".encode())
